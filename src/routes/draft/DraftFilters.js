...
class DraftFilters extends Component {
  render() {
    ...
    const top_level_tabs = Object.keys(filters).map(filter => (
      <Tab label={filter} value={filter} />
    ));

    const lower_level_tabs = filters[top_level_filter_selected].map(filter => {
    ...
    let logo = team => (
      <img
        className={s.logo}
        src={`https://static.nfl.com/static/site/img/logos/svg/teams/${team}.svg`}
      />
    );
